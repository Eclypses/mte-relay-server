...
export async function instantiateMteWasm(options: {
  ...
  // check for TRIAL build
  const drbg = mteBase.getDefaultDrbg();
  const drbgMin = mteBase.getDrbgsEntropyMinBytes(drbg);
  const drbgMax = mteBase.getDrbgsEntropyMaxBytes(drbg);
  isTrial = drbgMin === 0 && drbgMax === 0;
  if (isTrial) {
    let msg =
...
export async function instantiateEncoder(options: {
  ...
  const initResult = encoder.instantiate(SECRET + options.personalization);
...
export async function instantiateDecoder(options: {
  ...
  const initResult = decoder.instantiate(SECRET + options.personalization);
