 ...
 */
const mteIdManager: FastifyPluginCallback = async (fastify) => {
  ...
  fastify.addHook("onRequest", (request, reply, _done) => {
    try {
      ...
      if (SETTINGS.OUTBOUND_TOKEN) {
        ...
        const outboundToken = request.headers[
          SETTINGS.OUTBOUND_TOKEN_HEADER
        ] as string;
        if (outboundToken) {
          if (outboundToken !== SETTINGS.OUTBOUND_TOKEN) {
            request.log.error(`Invalid outbound token: ${outboundToken}`);
      ...
      const mteRelayHeader = request.headers[
